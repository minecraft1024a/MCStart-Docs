...
## 实例更新
暂未实现，而且不推荐使用，想升级实例最好再重新部署一个，手动一条一条的粘贴配置参数（因为版本升级后，配置参数往往会有变化）和数据文件夹（即data文件夹，位于本体根目录下，直接复制粘贴到新实例本体根目录下即可）。

## 实例删除
选项`[C] 实例删除`

这个功能是通过完全删除实例所有组件的根目录来达到释放实例的目的的。

选择这个选项后，会要求你通过输入实例序列号的方式来确定要删除的实例。确定后，会要求你再次输入实例序列号并输入`DELETE`（这是为了防止误触的），如果输入正确，程序会启动实例释放流程，我们会询问你是否删除实例根目录下的组件（如NapCat，适配器等），<mark>是否保留config文件夹（存储实例配置）和data文件夹（存储实例数据）</mark>，并最终删除实例根目录下的所有文件及文件夹。